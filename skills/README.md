...
Agent 应优先使用主 Agent 原生 subagent 工作流：主 Agent 选取 focus、派发 coder、读取 result、运行验证、审计 diff 并合并 state。CLI 只辅助 session 管理、state 校验、finalize 和 protocol-only session 骨架生成；旧 `--run`、`--check`、`--dispatch` 外部 Worker 路径已废弃。
...
fastcar-cli auto-iterate --verify --from docs/prd.md --session login-verify --yes
fastcar-cli auto-iterate --plan-only --goal "设计支付模块" --session payment-plan --yes
...
session 启动路径会生成 session 文件和当前活动指针：
...
默认原生 subagent 模式下不得启动旧 `--run` pipeline；主 Agent 直接读取 skill/state 并执行裁判职责。用户明确要求 protocol-only / LLM-only、手动模式或不启动 subagent 时，启动命令追加 `--no-run`，由当前 LLM 遵循自动迭代技巧执行。
...
```bash
# 严格启动：完整流程清单
fastcar-cli auto-iterate --strict --from docs/prd.md --session prd-impl --yes
...
# Protocol-only / LLM-only：不启动 subagent，由当前 LLM 遵循自动迭代技巧执行
...
# 安装到指定 Agent
fastcar-cli skill install auto-iterate-coding --target <agent>
fastcar-cli skill install auto-iterate-coding --target codex
```

当前架构说明见仓库根目录的 `docs/auto-iterate-current-architecture.md`；skill 内不再维护 compatibility/adapters 旧文档层。

## 自然语言触发示例

如果 Agent 已读取 `auto-iterate-coding`，可以直接对 Agent 说下面这些大白话。Agent 应默认进入主 Agent + coder subagent 原生工作流；CLI 仅用于 session 管理、state 校验、finalize 或 `--yes` session 骨架。只有用户明确要求 protocol-only / 手动模式 / 不启动 subagent 时才追加 `--no-run`。
