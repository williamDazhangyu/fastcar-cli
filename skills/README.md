...
Agent 应优先转换为 CLI 驱动自动模式：先检查 Worker，再由 CLI 持有主循环、状态、验证和停止条件。

```bash
fastcar-cli auto-iterate --check --json-progress
fastcar-cli auto-iterate --run --autopilot --quick --goal "修复登录失败" --session login-bugfix --json-progress
...
fastcar-cli auto-iterate --run --once --plan-only --goal "订单模块重构" --session order-plan --json-progress
fastcar-cli auto-iterate --resume login-bugfix --run --autopilot --json-progress
fastcar-cli auto-iterate --list
```

只有 Worker 不可用、CLI flag 不支持、运行环境不能 spawn Worker，或用户显式要求手动 / `--no-run` 时，才使用 fallback 启动文件路径：
...
fastcar-cli auto-iterate --plan-only --goal "设计支付模块" --session payment-plan --yes --no-run
```
...
fallback 启动路径会生成 session 文件和当前活动指针：
...
自动模式下不得要求用户复制 `start-prompt.md`；Router 只读取 `--json-progress` NDJSON 事件，并在 `need_decision` 时询问用户。只有 fallback 模式才读取 `.agent-state/auto-iterate/<session>/start-prompt.md`。
...
fastcar-cli auto-iterate --run --autopilot --quick --goal "修复登录失败问题" --session login-bugfix --autopilot-max-iterations 5 --json-progress
...
如果 Agent 已读取 `auto-iterate-coding`，可以直接对 Agent 说下面这些大白话。Agent 应默认路由为 `--check --json-progress` 后接 `--run --json-progress`；只有 fallback 启动命令才追加 `--yes --no-run`。
