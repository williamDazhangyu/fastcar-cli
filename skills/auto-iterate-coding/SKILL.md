...
description: 面向 AI Coding Agent 的有界自动迭代开发协议。Use when Codex, Kimi Code CLI, Claude Code, Copilot, Cursor, Gemini CLI or another AI coding agent needs to implement, debug, repair, verify, or recursively optimize code through bounded edit-test-fix loops with progress tracking, real validation, Mermaid visualization, architecture-friction detection, safety controls, state handoff, and graceful degradation when tool capabilities are unavailable.
---
