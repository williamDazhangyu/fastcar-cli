...
Codex 客户端 Goal 入口、普通提示词中的 `Goal:` 前缀、以及 `fastcar-cli auto-iterate --goal` 是三种不同概念。本协议不能通过提示词或 `--goal` 参数强制启用 Codex 客户端 UI 的 Goal 模式；只能在客户端已提供 Goal 入口时建议用户把自动迭代启动提示放入该入口。CLI 或普通聊天环境中，`Goal:` 只是普通文本前缀，fastcar-cli 最多把它清洗为目标文本；`--goal` 只是 fastcar-cli 的目标参数。具体路由边界见 [references/natural-language-routing.md](references/natural-language-routing.md) 的 “Goal 术语边界”。
