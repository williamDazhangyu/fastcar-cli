...
- 当前 `mode`、`session`、`state.md`、`auto-iterate-current.json`。
- 状态持久化能力：`available` / `degraded` / `not_available`。
- 下一步最小动作，例如读取 state、reconcile、能力探测、提取 RCM 或建立 feedback loop。

如果当前环境不能运行 CLI，但能写文件，使用无 CLI fallback 创建 `.agent-state/auto-iterate/<session>/state.md`、`start-prompt.md` 和 `auto-iterate-current.json` 后再声明激活。如果不能写这些文件，必须声明 `状态持久化：not_available`，并在对话内维护同等结构；不得把这种降级状态描述为完整持久化 session。

示例：

```text
auto-iterate 已激活：mode=quick，session=login-bugfix。
state=.agent-state/auto-iterate/login-bugfix/state.md，current=.agent-state/auto-iterate-current.json，状态持久化=available。
...
- 写入和读取 `.agent-state/auto-iterate/<session>/state.md`。
- 使用子 Agent、并行探索或后台任务。
...
每轮并发 dispatch 前必须由父 Agent 建立轻量 baseline（例如 git status、已有 diff 摘要、关键文件 mtime 或等价审计信息），并在子 Agent prompt 中声明不得读写 `.agent-state/`。merge 后必须由父 Agent 执行 Quality Gate、更新 `active_sub_agents` / `sub_agent_history` 和全局状态。如果 state.md 在子 Agent 运行期间被外部修改，必须先进入 `reconcile`，不得继续 dispatch。
...
- 启动 Autopilot、medium/large 或多轮自动迭代前，必须先确认或创建 `auto-iterate/<session>/state.md`、`auto-iterate/<session>/start-prompt.md` 和 `auto-iterate-current.json`。
...
- 用户回答后立即记录到 `state.md` 的 Decisions 或对话内状态，再进入下一个问题或继续实现。
...
10. 更新 `state.md`：At-a-Glance、Iteration State、Watchdog、Requirement Coverage Matrix 和 Definition of Done。
11. 提取关键失败信号
12. 基于失败信号继续迭代
13. 验证通过或触发停止条件
14. 验证通过后，执行有边界的递归优化
15. 输出交付总结、验证证据、风险和可选 Mermaid / 纯文本进度图
```
...
- 新功能和缺陷修复优先使用垂直切片 TDD。一次写一个外部行为测试或等价验证，再做最小实现；不要先批量写完所有测试再一次性实现。
...
- 是否正在累积没有验证价值的 patch。
- 是否已经触发停止条件。
- 看门狗是否触发 `run_validation`、`reconcile`、`ask_user` 或 `stop`。
...
Autopilot 或复杂任务应优先把可恢复状态保存到项目内 `.agent-state/auto-iterate/<session>/state.md`。该文件用于跨会话恢复，不替代真实代码检查和验证。

独立 session state 是当前 schema 的唯一权威状态源；legacy mirror 只能作为兼容旧流程的人类摘要，不得作为 Autopilot、多轮任务或并行任务的唯一状态源。
...
启动或恢复时如果存在当前 session 的 `state.md` 且当前环境可读取，先读取它作为任务恢复状态；继续时不要依赖历史对话，只依赖状态文件、当前代码和真实验证结果。
...
优先从 [references/INDEX.md](references/INDEX.md) 选择当前阶段需要的文件，不要一次性加载全部 references。
