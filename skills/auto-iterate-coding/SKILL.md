...
description: '面向 AI Coding Agent 的有界自动迭代开发协议。Use when the user says 自动迭代、全自动开发、快速启动、一直修到通过、验收 PRD、检查是否完成、只规划不要写代码、优化模块、恢复 session、切换 session、列出自动迭代任务, or when an AI coding agent needs to route natural language into fastcar-cli auto-iterate commands, or implement, debug, repair, verify, plan, or recursively optimize code through bounded edit-test-fix loops with progress tracking, real validation, state handoff, and graceful degradation when tool capabilities are unavailable. Default execution uses native coder subagents when available: the main Agent acts as judge, dispatches coder, validates result, runs verification, merges state, manages watchdog/budget/delivery gates, and asks user on need_decision. When the user explicitly requests `--no-run`, manual mode, no subagent, or protocol-only, the current LLM follows auto-iterate techniques directly without dispatching subagents. External CLI Worker (`fastcar-cli auto-iterate --run`) is deprecated.'
---
...
平台适配原则：把 `fastcar-cli`、`Agent(subagent_type="coder")`、Node runner、git diff、Codex `/goal` 等视为当前项目或平台的实现方式；移植到其他 Agent 或仓库时，必须使用等价的 session 管理、子任务派发、命令执行、diff 审计和目标状态能力。缺少某项能力时降级为 `not_available`、`not_verified` 或 `blocked`，不得把示例工具名当作协议硬前提。

## 执行路径识别

当前环境必须在启动或恢复时锁定下面两种执行风格之一，不得运行中静默切换：

**A. 主 Agent 直接管理 Subagent（默认）**：主 Agent 派发 coder subagent 编码，校验由主 Agent 自己用工具事实完成（例如 Node runner exit code / git diff / JSON schema，或平台等价证据）。主 Agent 不亲自写代码。

**B. Protocol-only / LLM-only（`--no-run`）**：用户显式要求手动模式、不启动 subagent、protocol-only，或当前环境没有原生子 Agent 能力时使用。若 `fastcar-cli` 可用，调用 `fastcar-cli auto-iterate --quick --yes --no-run` 生成 protocol-only state 骨架；若不可用，按无 CLI fallback 维护等价状态。当前 LLM 在当前对话中遵循自动迭代技巧执行，不使用主 Agent / Coder Subagent 角色边界，不派发 `Agent(subagent_type="coder")` 或等价 coder。
...
当前默认执行模式。主 Agent 派发 coder subagent 编码，校验由主 Agent 自己用工具事实完成（Node runner exit code、git diff、JSON 内容或平台等价证据），避免用推理替代验证。
...
- `coder` 是唯一允许修改本轮 scope 内业务代码的角色；完成修改后必须写本轮 `result.json` 并停止。coder 不得运行任何命令，不得写 `.agent-state/` 下除本轮 `result.json` 以外的文件。
- 每轮只派发一个 coder；coder 写入 `result.json` 后停止，主 Agent 校验后再决定下一轮。
- 校验靠工具事实，不靠推理。优先使用 Node 确定性 runner、`git diff --name-only` 和 JSON schema；其他平台必须提供等价的命令结果、diff/scope 审计和 schema 证据。验证命令不得经 shell 字符串解释。
- 防偷懒机制：每轮校验后写 `validation.log`，交付前主 Agent 必须逐条确认命令真实执行过（exit code 有值、duration > 0）。
- 需求标记为 `passed` 必须有主 Agent 的真实验证证据（CLI runner、测试命令、结构化检查或平台等价结果）。`scope violation` 拒绝本轮。
...
目标状态边界：Codex goal 模型、普通提示词中的 `Goal:` 前缀、以及 `fastcar-cli auto-iterate --goal` 是三种不同概念。Codex goal 模型是 Codex 运行时提供的可选目标状态能力；非 Codex 平台应使用自身等价能力或跳过该增强。交互式 Codex 中通过输入 `/goal` 使用该入口；本地 Codex 可通过 `codex features list` 中的 `goals stable true` 辅助确认 goal feature 是否启用，但不应期待 `codex goal` 子命令。CLI 或普通聊天环境中，`Goal:` 只是普通文本前缀，fastcar-cli 最多把它清洗为目标文本；`--goal` 只是 fastcar-cli 的目标参数，不会创建或更新 Codex goal。推荐配合方式是先用 `/goal` 设置 Codex 会话级整体目标，再用 `fastcar-cli auto-iterate --goal` 创建可恢复 session；`/goal` 不替代 `.agent-state/auto-iterate/<session>/state.json` 中的预算、RCM、验证证据、恢复状态和交付门禁。具体路由边界见 [references/natural-language-routing.md](references/natural-language-routing.md) 的 “Goal 术语边界”。
...
- 恢复任务、切换 session、列出自动迭代任务、查看会话、查看进度、打开 dashboard、resume session、switch session、list session、dashboard session。
...
- 只读探索、需求拆解、架构理解、能力探测、状态恢复、reconcile、交接摘要/新视角复核、向用户提问、生成计划和读取日志，不计入实现迭代轮次。
...
Agent 按以下 9 步主动引导用户完成决策确认。先执行 QA-style 轻量澄清：保留用户原话，最多问 2-3 个短问题补齐 expected / actual / repro / acceptance impact；信息足够时直接提取 RCM，不要过度 interview。完整流程见 [references/grill-session.md](references/grill-session.md)：
...
7. **术语对齐**：建立共享语言，从项目文档和代码中提取关键术语并确认定义。详见 [references/domain-language.md](references/domain-language.md)。
8. **架构决策**：确认代码落点和架构约束。
...
- 主 Agent 亲自执行 schema 校验、Node runner 或等价验证、git diff/scope 或等价审计、`validation.log` 写入、state merge、预算和交付门禁。
...
- 路径 A（默认）：主 Agent 使用 `Agent(subagent_type="coder")` 或平台等价能力直接派发 coder subagent，自身持有完整主循环。`fastcar-cli` 只提供 session 管理和 state 骨架生成。
...
- 实现需求的模式中，功能实现并通过验证后、交付前必须执行 Style Consolidation / 技巧风格整理：读取本项目 `.agents/skills` 和全局 skills 中相关代码风格、框架/API/SDK 约束、TypeScript 规范、反模式和验证建议，只整理本次修改范围内代码，并重新运行相关验证；不得扩大行为范围、引入无关重构或削弱测试。
...
- 启动并维护看门狗状态；每轮迭代、交接摘要/新视角复核、恢复和交付前都必须检查是否超时、无进展、状态漂移、验证缺失或交付不可验证。
...
- `fresh_eyes_required` 是否为 true。当所有关键 REQ passed 且 remaining_implementation_iterations > 0 时，本字段必须设为 true。若为 true 且未处理，required_action = context_compress_and_review。该机器枚举表示 auto-iterate 的交接摘要与新视角复核，不表示触发运行时上下文压缩。
...
- `context_compress_and_review`：所有 REQ 已 passed 但仍有剩余预算。生成 Context Handoff Summary（交接摘要），只保留可执行事实，主动丢弃对话中的实现细节偏见。以"新接手项目的开发者"视角重新审视全部代码和 RCM。发现遗漏 → 创建新 REQ，重置 fresh_eyes_required = false，继续迭代。无遗漏 → fresh_eyes_required = false，继续优化或交付。注意：这是 auto-iterate 状态交接动作，不是要求触发 Agent 运行时上下文压缩。
...
Autopilot 或长任务中，不要依赖完整对话历史继续推进。上下文变长、进入新阶段或需要 fresh-eyes 复核时，必须生成可执行的 Context Handoff Summary（上下文交接摘要）。

这里的“交接摘要”是 auto-iterate 的状态管理动作：把用户目标、RCM、验证证据、关键 diff、风险和下一步写成可恢复摘要。它不是要求调用或暗示 Agent 运行时的 context compaction / context compression 功能，也不得要求输出模型私有 chain-of-thought。

满足以下任一条件时更新 Context Handoff Summary：
...
Context Handoff Summary 输出结构以 [examples/state-template.md](examples/state-template.md) 的 `## Context Handoff Summary` 为唯一来源；不要在 `SKILL.md` 中维护第二份字段模板。
...
- `summary` 优先描述用户可见行为；文件路径、函数、类名和实现细节放入 `relatedFiles`、`evidence` 或 `nextStep`，不要作为需求摘要主体。
- 从模糊问题或 bug 中提取 REQ 时，先记录用户原话，再补齐 `expectedBehavior`、`actualBehavior`、`reproSteps` 和 `acceptanceImpact`；缺失字段可标记为待确认。
- 独立失败模式、独立验收路径、可并行关注点或明确依赖边界必须拆成不同 REQ；同一用户可见行为且共享验收路径的多个症状应合并。
- 每条 REQ 必须足够薄，能独立推进、独立验证、独立标记状态；必要时记录 `dependsOn`、`blockedBy` 和 `canStartImmediately`。
...
每次任务交付、提前停止或阶段性验收后，Agent 必须执行 `Skill Capture / 技能沉淀`。目标是在本项目 `.agents/skills` 中沉淀可迁移、可行动、可验证的技能点，并维护 `.agents/skills/index.md` 作为检索入口；完整规则见 [references/skill-capture.md](references/skill-capture.md)。

核心门禁：只沉淀可迁移、可行动、可验证的技能点；每条技能点压缩为 Trigger / Signal、Do、Verify、Avoid、Boundary、Source Evidence；不要记录密钥、客户数据、一次性日志、完整报错堆栈或大段源码；没有高价值内容时写 `skipped_no_high_value`。`deliveryEvidence.status=ready / delivered` 前，`skillCapture.status` 不得为 `pending`；`captured` 时必须记录 `capturedFiles` 并确认 `.agents/skills/index.md` 已同步。
