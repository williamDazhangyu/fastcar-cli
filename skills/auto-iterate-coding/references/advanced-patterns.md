...
原型必须先写清楚要回答的一个问题。一个原型同时回答状态机、UI 信息架构、性能和持久化方案，通常说明 scope 太大，应拆成多个原型或回到 Grill Session。
...
交接规则：原型结论只引用可复现命令、截图路径、验证输出、保留文件和用户选择；不要把完整探索过程复制进 handoff 或最终交付。
...
记录规则：最终 state、handoff 和交付摘要必须分别记录 Standards 发现和 Spec 发现，不要合并成一个"review passed"结论。每个发现都要指向依据、影响和建议动作。
