...
- **父 Agent 是唯一协调者**：子 Agent 只执行委派任务并返回结果，不直接修改 state.md、不直接做跨任务决策、不创建独立 session。
...
2. 父 Agent 写入 state.md：`active_sub_agents` 包含 3 个 explore 任务，并记录当前 `current_phase=explore`。
...
idle
  ...
  -> dispatched   active_sub_agents 已写入 state.md
...
4. write planned dispatch to state.md
...
- 如果检测到 state.md 在子 Agent 运行期间被外部修改，必须标记 `state_drift=confirmed` 并进入 reconcile。
...
- 父 Agent 已在 Architecture Confirmation 中完成文件落点确认（记录在 state.md `## Decisions`）。

安全前提（必须全部满足，否则串行执行）：

1. 父 Agent 已完成架构确认和文件拆分，并写入 state.md `## Decisions`。
2. 每个 coder 子 Agent 分配的文件集合互不相交。
3. 子 Agent 禁止直接写入 state.md —— 只有父 Agent 有写入权。子 Agent 也不得读取 `.agent-state/` 下任何文件。
4. 所有子 Agent 完成后，父 Agent 执行 reconcile → 统一验证 → 合并状态。
5. 并行写入分两档安全边界：
   - **档一（有隔离）**：项目使用了 git worktree 或等效 patch bundle 隔离机制 → 允许 coder 子 Agent 并发写入。
   - **档二（无隔离）**：项目在同一 worktree 下、无 patch 隔离 → 默认只允许 explore 并发和 verify 并发；coder 并发写入需要用户在 state.md `## Decisions` 的并发决策中显式确认允许同 worktree 并发写入，且父 Agent 必须记录无隔离风险和回退方案。
...
- 统一运行所有验证命令（并行验证阶段三）。
- 更新 state.md：`## Current State`、`## Requirement Coverage Matrix`、`## Watchdog`、`## Definition of Done`。
...
- **禁止访问的文件**：显式列出 `.agent-state/` 目录下的所有文件（state.md、start-prompt.md、auto-iterate-current.json）为禁止读写。
- **输出格式要求**：规定子 Agent 返回的摘要格式。
- **禁止事项**：不得修改 state.md、不得写入密钥/token、不得执行破坏性 git 命令、不得新增依赖、不得创建或切换 session。
- **超时预期**：避免子 Agent 无限等待。

### 委派 Prompt 模板

父 Agent 可以按以下模板委派 explore、coder 或 background 子任务。模板中的字段必须由父 Agent 填实，不能让子 Agent 自行读取 state.md 补全。
...
- 子 Agent 如果主动读取 state.md 并根据其内容改变行为，属于协议违规；子 Agent 只依据父 Agent 在 prompt 中给出的任务描述和约束执行。
...
- 子 Agent 超时/崩溃/返回不完整结果 → 父 Agent 在 `## Sub-Agent Dispatch` 中标记 `failed`，不合并该子 Agent 的结果，不更新 state.md 中的需求状态。
...
- 每轮 merge 后，父 Agent 更新 state.md 的全局进度（At-a-Glance / Current State / Watchdog / Budgets），当前轮的子 Agent 结果移入 `sub_agent_history`，清空 `active_sub_agents`。
...
`fastcar-cli auto-iterate --validate-state [session|state.md]` 是只读校验入口，用于检查完整 auto-iterate session 基线和 Agent 工具执行并发协议后留下的 session state 是否存在明显违规。它不负责启动、停止或调度子 Agent，也不替代父 Agent 的 Quality Gate。

session 基线校验覆盖：

- 18 个必需章节是否存在。
- `state.md`、`start-prompt.md`、`auto-iterate-current.json` 是否存在且指向同一 session。
