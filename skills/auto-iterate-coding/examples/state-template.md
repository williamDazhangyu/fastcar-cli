...
说明：从项目 AGENTS.md、README、docs/、代码中提取的共享术语，用于 Agent 和用户使用同一套词汇沟通。
提取来源：
术语表：
| 术语 | 英文 | 定义 | 来源 | 备注 |
|------|------|------|------|------|
| | | | | |
使用规则：Agent 在命名、描述需求、输出交付总结时必须使用术语表中的词汇。
...
每次任务交付、提前停止或阶段性验收后，必须执行 Skill Capture / 技能沉淀：把可迁移、可行动、可验证的经验按结构化模板写入 .agents/skills，并维护 .agents/skills/index.md；没有高价值内容时记录 skipped_no_high_value 和原因。
```
