...
1. 主 Agent 读取 skill 和 session state，直接派发 `Agent(subagent_type="coder")`；coder 只执行单轮编码并写 `result.json`。
...
```bash
fastcar-cli auto-iterate --diagnose --goal "诊断当前 e2e 偶发失败" --session flaky-e2e --autopilot-max-iterations 3 --yes
```
...
## Protocol-only / LLM-only 补充

只有用户显式要求 protocol-only / LLM-only、手动模式、不启动 subagent，或当前环境没有 `Agent(subagent_type="coder")` 工具时，才使用 `--no-run`。该路径不是 agent/subagent 拓扑；当前 LLM 自己遵循自动迭代技巧执行，例如：

```bash
fastcar-cli auto-iterate --quick --goal "修复登录失败" --session login-bugfix --autopilot-max-iterations 5 --yes --no-run
```

此时当前 LLM 读取 `.agent-state/auto-iterate/<session>/start-prompt.md`，并在同一会话里维护 state、RCM、DoD、Watchdog、验证记录和停止条件，不启动 subagent。
