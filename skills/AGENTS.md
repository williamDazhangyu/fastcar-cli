...
- Agent 实现问题时必须优先从通用机制、可复用扩展点和现有架构一致性出发，不得为了某个特定问题写强耦合、硬编码或一次性特定解。
- 修复 bug 时应先分析同类问题是否存在共性，优先修复根因、抽象边界或公共流程；只有明确证明影响范围极小且无通用价值时，才允许局部修复，并说明原因。
- 新增逻辑应优先通过配置、接口、策略、类型约束或公共工具函数表达通用能力，避免把具体 case、具体文案、具体路径或具体测试数据写死在业务流程中。
- 测试应覆盖通用行为和边界条件，不应只为当前失败样例写“刚好通过”的断言。
- 测试设计、失败判断和修复验收必须基于真实运行结果、日志、断言、diff 或可复现步骤等证据，不得凭感觉推测问题已覆盖或已修复。
