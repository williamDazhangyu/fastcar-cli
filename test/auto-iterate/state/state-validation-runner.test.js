...
test("strict mode reports historical bloat as warning when baseline did not worsen", async () => {
  await withTempCwd(async () => {
    writeValidSession();
    fs.mkdirSync("src", { recursive: true });
    fs.mkdirSync("test", { recursive: true });
    fs.writeFileSync(path.join("src", "index.ts"), "a\n");
    fs.writeFileSync(path.join("test", "index.test.ts"), "t\n");
    writeJson(".agent-state/auto-iterate/demo/state.json", {
      ok: true,
      bloatBaseline: {
        testLines: 2,
        srcLines: 2,
    ...
    const result = await validateState("demo", { strict: true, silent: true }, schemaOk, process.cwd());

    assert.strictEqual(result.ok, true);
    assert(result.issues.some((issue) => issue.severity === "warning" && issue.message.includes("历史膨胀债务未恶化")));
  });
});

test("strict mode blocks bloat that worsened after baseline", async () => {
  await withTempCwd(async () => {
    writeValidSession();
    fs.mkdirSync("src", { recursive: true });
    fs.mkdirSync("test", { recursive: true });
    ...
    assert.strictEqual(result.ok, false);
    assert(result.issues.some((issue) => issue.severity === "error" && issue.message.includes("本次新增膨胀阻断")));
  });
});
