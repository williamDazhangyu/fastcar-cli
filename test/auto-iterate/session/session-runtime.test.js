...
test("initAutoIterate routes --check-bloat as full repository diagnostic", async () => {
  await withTempCwd(async () => {
    ...
    process.exitCode = 0;
    try {
      const { lines } = await captureConsole(() => initAutoIterate(["--check-bloat"]));
      const output = lines.join("\n");

      assert.strictEqual(process.exitCode, 1);
      assert(output.includes("Skills:"));
      assert(output.includes("Tests:"));
      assert(output.includes("测试占比超限"));
      assert(!fs.existsSync(".agent-state"));
    } finally {
      process.exitCode = previousExitCode;
    }
