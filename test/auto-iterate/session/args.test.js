...
test("parseArgs parses loop helper flags without polluting inferred goals", () => {
  const next = parseArgs(["--next", "loop-session"]);
  assert.strictEqual(next.nextSession, "loop-session");
  assert.strictEqual(next.goal, null);

  const merge = parseArgs(["--merge", "loop-session", "--round", "2"]);
  assert.strictEqual(merge.mergeSession, "loop-session");
  assert.strictEqual(merge.mergeRound, 2);
  assert.strictEqual(merge.goal, null);
});

test("parseArgs records deprecated external worker flags", () => {
  const run = parseArgs(["--run", "--quick", "--goal", "legacy"]);
  assert.strictEqual(run.deprecatedFlag, "--run");
  assert.strictEqual(run.mode, "quick");
  assert.strictEqual(run.goal, "legacy");

  const dispatch = parseArgs(["--dispatch=demo"]);
  assert.strictEqual(dispatch.deprecatedFlag, "--dispatch");
  assert.strictEqual(dispatch.goal, null);
});
