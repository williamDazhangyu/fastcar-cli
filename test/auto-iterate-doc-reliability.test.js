...
test("examples 命令输出 Codex worker / dispatch 派发自然语言示例", () => {
  ...
  assertIncludes(output.stdout, "Codex /goal 与 worker dispatch", "examples stdout");
...
test("任务后技能沉淀写入 .agents skills 并维护 index 入口", () => {
  ...
  assertIncludes(deliveryTemplate, "contextResetReview.status=passed", "delivery-template.md");
