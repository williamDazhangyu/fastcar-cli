...
test("state-schema、state-template 与 CLI 初始 state 的必需章节保持一致", () => {
  ...
  assert.strictEqual(requiredSections.length, 30);
...
test("auto-iterate session 生成机器权威 state.json 和 Markdown 视图", () => {
  ...
  assertIncludes(stateJson.postAgentValidationGate.command, "--validate-state", "postAgentValidationGate.command");
  assertIncludes(stateJson.postAgentValidationGate.command, "--strict-state", "postAgentValidationGate.command");
  assert.strictEqual(
  ...
  assertIncludes(state, "command：fastcar-cli auto-iterate --validate-state state-json-check --strict-state", "state.md");
...
test("references INDEX 索引的文档真实存在并覆盖关键模式组合", () => {
  ...
  assertIncludes(index, "delivery-template.md", "INDEX.md");
...
test("任务后技能沉淀写入 .agents skills 并维护 index 入口", () => {
  ...
  const legacyOpenaiAdapter = readRepoFile("skills/auto-iterate-coding/adapters/openai.yaml");
  ...
  assertIncludes(deliveryTemplate, "skillCapture.status", "delivery-template.md");
  ...
  assertIncludes(openaiAdapter, "tool_mapping:", "adapters/openai.yaml");
  assertIncludes(openaiAdapter, "platform_constraints:", "adapters/openai.yaml");
  assertIncludes(openaiAdapter, "prompt_contract:", "adapters/openai.yaml");
  assertIncludes(openaiAdapter, "state_handling:", "adapters/openai.yaml");
  assertIncludes(openaiAdapter, "Prefer `index.md` for navigation", "adapters/openai.yaml");
  ...
  stateJson.skillCapture.lastRunSummary = "已沉淀技能索引更新规则";
  fs.writeFileSync(paths.stateJson, `${JSON.stringify(stateJson, null, 2)}\n`, "utf8");
