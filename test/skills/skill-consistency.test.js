...
test("AGENTS.md 声明主 Agent 与 Coder 硬边界", () => {
  for (const file of ["AGENTS.md", "skills/AGENTS.md"]) {
    ...
    assert.ok(agents.includes("Subagent 驱动架构"), file);
    assert.ok(agents.includes("主 Agent 作为裁判"), file);
    assert.ok(agents.includes("coder subagent"), file);
    assert.ok(agents.includes("--no-run"), file);
    assert.ok(agents.includes("protocol-only / LLM-only"), file);
    assert.ok(agents.includes("auto-iterate-current-architecture.md"), file);
    assert.ok(agents.includes("auto-iterate-coding/SKILL.md"), file);
    assert.ok(agents.includes("已完全移除"), file);
    assert.ok(agents.includes("next focus"), file);
    assert.ok(agents.includes("validation.log"), file);
    assert.ok(agents.includes("merge state"), file);
    assert.ok(agents.includes("check-bloat"), file);
    assert.ok(agents.includes("测试膨胀"), file);
  }
});
...
test("主 Agent 裁判 runbook 不引入 native runtime adapter", () => {
  ...
  assert.ok(judgeRunbook.includes("fastcar-cli auto-iterate --next <session>"));
  assert.ok(judgeRunbook.includes("fastcar-cli auto-iterate --merge <session> --round <n>"));
...
test("pipeline 测试已按 focus、schema 和 validation 职责拆分", () => {
  const mergeState = read("test/auto-iterate/pipeline/merge-state.test.js");
  const iterationPrompt = read("test/auto-iterate/pipeline/iteration-prompt.test.js");
  const focusLoop = read("test/auto-iterate/pipeline/focus-loop.test.js");
  const resultSchema = read("test/auto-iterate/pipeline/result-schema.test.js");
  const validation = read("test/auto-iterate/pipeline/validation.test.js");
  ...
  assert.ok(packageJson.includes("node test/auto-iterate/pipeline/focus-loop.test.js"));
  assert.ok(packageJson.includes("node test/auto-iterate/pipeline/result-schema.test.js"));
  assert.ok(packageJson.includes("node test/auto-iterate/pipeline/validation.test.js"));
  assert.ok(packageJson.includes("node test/auto-iterate/pipeline/pick-focus.test.js"));
  assert.ok(packageJson.includes("node test/auto-iterate/pipeline/merge-state.test.js"));
  assert.ok(packageJson.includes("node test/auto-iterate/pipeline/delivery-gates.test.js"));
  assert.ok(packageJson.includes("node test/auto-iterate/pipeline/watchdog.test.js"));
  assert.ok(packageJson.includes("node test/auto-iterate/pipeline/iteration-prompt.test.js"));
  assert.ok(packageJson.includes("node test/auto-iterate/pipeline/progress.test.js"));
});
