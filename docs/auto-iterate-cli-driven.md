...
| 多轮 pipeline | `loopPolicy.js` 集中解析 `once/plan/autopilot/maxSteps` loop shape；`runPipeline` 写入 `mode.runtimeAutopilot` 和 `mode.loopShape`；需求关闭后不再直接成功，先输出 `delivery_gate`；Router UX 规则已代码化覆盖 §13.2 五个场景和 flag registry；Kimi 真实 Router UX smoke 已通过 | ⚠️ 部分交付 | Claude 真实终端端到端 UX 和 Claude/Gemini/Cursor native Worker 集成仍未完整稳定 |
...
| 写范围保护 | `writeGuard` 已实现 verify 默认禁止写、plan 禁止写、prototype 默认 `prototype/**`、`.agent-state/` 禁写、`--scope` 范围检查、`--allow-modify` 放行 verify 写入 | ✅ 已交付 | isolate 冲突恢复仍需更多测试 |
| `--isolate` | 已实现 git worktree 创建、Worker 隔离运行、diff apply 回主工作区、清理 worktree；merge 冲突会保留 worktree、写 state.isolate 并输出 `worktree_merge_failed`；Windows 单测和 Linux 远程 smoke 均已覆盖 worktree 创建/合并/清理 | ⚠️ 部分交付 | 真实 native Worker 行为仍需更多测试 |
...
| 测试 | `npm test` 已串起 6 个测试文件；`pipeline.test.js` 覆盖 34 个 pipeline 用例，含 loopPolicy、delivery_gate、no_progress、prompt、focus、baseline、need_decision、answer、writeGuard、scope、prototype、isolate 冲突、diagnose CLI 多轮、optimize CLI 多轮、diagnose hypothesisQueue、optimize metricComparison、result 缺失、worker 非零退出、invalid result、agent_timeout 和 Claude/Gemini/Cursor env-template smoke；`env-check.test.js` 覆盖 3 个环境检查用例；`adapters.test.js` 覆盖 15 个 adapter 用例；`router-ux.test.js` 覆盖 7 个 Router UX / flag registry 用例；Windows 与远程 Linux 全量测试均已通过 | ✅ 已交付 | 缺少 Claude 真实终端 UX 录制和 Claude/Gemini/Cursor native Worker CLI smoke |
...
| `--validate-cmd` | ✅ | ✅ | 覆盖 state 验证命令 | ✅ 已交付 |
| `--max-steps` | ✅ | ✅ | 控制循环上限 | ✅ 已交付 |
| `--step-timeout` | ✅ | ✅ | Worker timeout，默认 300 秒（代码实际默认） | ✅ 已交付 |
...
| `--scope` | ✅ | ✅ | 限制 `files_changed` 必须在 scope 内；help 已展示 | ✅ 已交付 |
...
当前白名单允许 `currentState`、`deliveryEvidence`、`notes`、`hypotheses`、`optimizationMetrics`；`notes` 追加到 `state.notes[]`，`hypotheses` 合并到 `state.diagnose.hypotheses[]` 和 `state.diagnose.hypothesisQueue[]`，`optimizationMetrics` 用于 baseline/post 指标对比。`validateStateJsonModel`、`state.schema.json`、`state-schema.md`、state-template 和测试已同步。
...
当前代码已解析并实现 `--scope`，`runPipeline` 也传入 `writeGuard` 生效；`node bin/cli.js auto-iterate --help` 和顶层 help 均已展示 `--scope <glob[,glob]>`。`state_patch.notes/hypotheses/optimizationMetrics`、`mode.runtimeAutopilot`、`mode.loopShape`、`traceability`、`documentation` 和 `deliveryDocs` 已同步 `validateStateJsonModel`、`state.schema.json`、`state-schema.md`、state-template 和测试。traceability 只保存公开可审计推理摘要，不记录私有 chain-of-thought；`--finalize` 生成 `.agent-state/auto-iterate/<session>/docs/{api.md,changelog.md,architecture.md,implementation.md}`。
...
| `agent_timeout` | Worker 超时或 hang | 杀掉本轮，输出 timeout 事件并停止本轮 | `agent_timeout` 后接 `error(reason=worker_failed)` |
| `invalid_result` | result.json 缺失、JSON 非法、schema 不符 | 丢弃 `state_patch`，必要时触发 watchdog | `error` |
...
1. 如果 options.resumeSession 存在且 options.run:
   ...
   c. 刷新 current 指针
   d. 如果带 --answer，则调用 applyDecisionAnswer 写入 pending decision
...
spawn Worker 阶段:
  ...
  - spawnSync timeout → emit agent_timeout，再 emit error(worker_failed)
  - spawnSync exit ≠ 0 → emit error(worker_failed) 并停止本轮

parse result 阶段:
  - result.json 缺失 → emit error(missing_result_json) 并停止，不合并 result
  - JSON 非法 → emit error(invalid_result_json) 并停止
  - schema 不符（resultSchema.js 校验不通过）→ emit error(invalid_result_json) 并停止，记录校验错误详情
...
| **optimize** | 是 | false | establish_baseline → optimize → verify_optimization | 全量但范围受 `--scope` 限制 | 必跑（baseline + post） | optimization 连续 2 轮无改善 → 停 |
...
- **optimize**：跑两次验证（baseline + post）并对比。`mergeIterationIntoState` 需要能存两份验证结果。
...
3. --resume + --run：先 activate/read session，再进入 runPipeline。
4. --resume 不带 --run：保持旧行为，只切换并输出恢复提示。
5. --run 不带 --resume：ensurePipelineSession；如果 session 不存在则按 --from/--goal/--session 创建初始状态。
...
**入参格式**：`commands` 是字符串数组 `string[]`。CLI 从 `state.validation.commands` 中提取每条 command 字段（当前 schema 结构为 `{ command, result, summary }[]`），取 `command` 字符串组成执行列表。
...
- 依次跑 `state.validation.commands` 中每一条命令。
- 任何一条 exit ≠ 0 → `passed: false`。
- stdout / stderr 各保留末 4KB 作为证据。
- 整体 timeout 默认 300 秒。
...
- 每轮 stdout / stderr 各保留末 4KB 进 `state.postChange.perCommand[].stdoutTail / stderrTail`。
...
`result.json` 是 Worker 与 CLI 的唯一结构化交接文件。Worker 输出是建议，CLI 合并才是真理。缺失或非法时不抛未捕获异常，而是分别输出 `error(reason=missing_result_json)` / `error(reason=invalid_result_json)` 并停止本轮；与实际 diff / 验证结果冲突时输出 `write_violation` / `watchdog_triggered` 等事件。`no_progress` 只消费 Worker 显式返回的合法 status。
...
| `state_patch` | 否 | 只允许建议 requirements evidence、notes、hypotheses 等白名单字段；不能直接改 budgets / watchdog / postChange。 |
...
`state_patch` 禁止包含：`budgets`、`watchdog`、`postChange`、`deliveryEvidence`、`skillCapture`、`updatedAt`、任何 `.agent-state/` 路径内容、密钥或完整日志。
...
| `postChange` | 单命令摘要 | 已新增 `perCommand[]`，保留每条验证命令的 status、exitCode、duration、stdoutTail、stderrTail | 兼容旧 `command/result/reason` 字段，新增字段向后兼容。 |
...
| `write_violation` | Worker 写了不被 mode 允许的文件 | `iter`, `mode`, `files`, `dropped` |
...
| `--scope <glob>` | 限定允许修改的文件范围；当前代码已解析并在 `writeGuard` 生效，但 help 文本未展示 | 无 |
...
| `--run --resume <session>` | 激活并读取现有 session，然后进入 pipeline；不得只打印恢复 prompt 后返回。 |
| `--run --validate-state` | 非法组合；`--validate-state` 是独立门禁命令，不进入 pipeline。 |
| `--run --dispatch` | 非法组合；legacy dispatch 与 pipeline worker 不是同一套生命周期。 |
| `--run --finalize` | 非法组合；pipeline 正常结束后可由 CLI 内部调用 finalize。 |
| `--run --capture-skills` | 非法组合；技能沉淀是收尾门禁，不是迭代主循环。 |
...
| 文件 | 实际行数 | 说明 |
|---|---|---|
| `src/pipeline/runPipeline.js` | 682 | 完整主循环：ensurePipelineSession 准备 session 后，runPipeline 执行 loop → spawn → validate → merge → emit（含 worktree 隔离、answer 恢复、delivery_gate 和 NDJSON 事件） |
...
| `test/pipeline.test.js` | 998 | 35 个 pipeline 集成测试，覆盖 loopPolicy、delivery_gate、no_progress、prompt、focus、baseline、need_decision、answer、writeGuard、scope、prototype、isolate 冲突、diagnose/optimize CLI 多轮、result 缺失、worker 非零退出、invalid result、`--focus` mode 过滤和 Claude/Gemini/Cursor env-template smoke |
| `test/router-ux.test.js` | 146 | 7 Router UX / flag registry 用例 |
| `test/adapters.test.js` | 215 | 15 adapter 契约测试，覆盖命令构造、超时、非零退出、result 缺失 |
| `test/env-check.test.js` | 93 | 3 环境检查测试，含 Cursor 官方 `agent` 二进制识别 |
| `test/auto-iterate-doc-reliability.test.js` | 2493 | 48 个 schema/模板一致性测试 |
| `test/skill-consistency.test.js` | 90 | 4 文档一致性测试 |
| `test/fixtures/pipeline-worker.js` | 136 | Worker 模拟 fixture |
...
4. ✅ 完整 `runPipeline`（682 行）+ `mergeState`（456 行）+ `shouldStop`（84 行）+ `pickFocus`（251 行，完整 focus 类型和 `--focus` mode 过滤）+ `progress` + `resultSchema`（含 `no_progress`）+ `envCheck`。
5. ✅ NDJSON 事件覆盖完整核心路径。
6. ✅ `test/pipeline.test.js`（998 行，35 个用例）+ `test/router-ux.test.js` + `test/adapters.test.js` + `test/env-check.test.js` + `test/skill-consistency.test.js`。
...
| Worker 输出 result.json 不合法 | `parseAndValidateIterationResult` 不抛异常；缺失或非法 result 当前输出 `missing_result_json` / `invalid_result_json` 并停止，合法 `no_progress` 才会累加 watchdog streak |
| `state.json` 字段太多，merge 容易漏 | 每轮 merge 后强制跑 `validateStateJsonModel`；CI 加单测 |
| 用户旧的 Agent 自治流程被破坏 | 默认行为不变；`--run` 是 opt-in |
| Worker 偷偷写 `.agent-state/` | WORKER.md 明确禁止；iterationPrompt 里再硬约束；`--isolate` 模式天然隔离 |
| 验证命令很慢导致每轮超时 | `--step-timeout` + `runValidationCommands` 独立 timeout；`--no-validate` 可临时跳过 |
...
| **Worker 超时 / 完全 hang** | `--step-timeout` 硬超时 + spawnSync timeout；超时打印 `agent_timeout` 事件 |
...
| **verify / plan 下 Worker 偏要写文件** | merge 随 deny-write 扫描 `report.files_changed`；违规 → `write_violation` 事件 + 本轮丢弃 + watchdog 报警。 |
...
`test/auto-iterate-doc-reliability.test.js`（约 2493 行）必须保持全部通过。PR-1/PR-2 期间新增的 pipeline 测试以独立文件 `test/pipeline.test.js` 和 `test/adapters.test.js` 存在，不与现有测试互相污染。
...
`--isolate` 启用时，`session_started` 事件增加 `isolated: true` 字段。每轮 worktree 创建/合并/清理失败时输出 `error` 事件，reason 为 `worktree_create_failed` / `worktree_merge_failed` / `worktree_cleanup_failed`。当合并失败时，CLI 保留隔离 worktree，事件中输出 `preserved_worktree`，并写入 `state.isolate.conflictWorktree` 与 `state.watchdog.requiredAction="stop"`。
