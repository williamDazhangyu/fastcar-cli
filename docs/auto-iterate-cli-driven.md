...
| 项目 | 当前代码事实 | 交付标注 | 仍需补齐 |
|---|---|---|---|
| 主入口与兼容 fallback 路径 | `parseArgs` 已支持全部 legacy flag 和 pipeline flag；`initAutoIterate` 中 `--run && !--no-run` 进入 `runPipeline`（约 680 行），不带 `--run` 仍走兼容 fallback 大 prompt 路径 | ✅ 已交付 | `--run` 非法组合已有保护，但 help/路由文档仍需持续同步 |
...
| `--no-run` | ✅ | ✅ | 压制 `--run`，回兼容 fallback 路径 | ✅ 已交付 |
...
| 兼容 fallback 与新 `--run` 路径共存时迁移边界不够硬 | PR 作者可能改坏现有 prompt 生成、resume、dispatch 或 finalize | §10.4 迁移与回滚规则 |
...
#### 历史问题 3：用户曾需手动复制 prompt，不是真正的「零人工」
...
| 维度 | 全量 prompt（历史 Agent 自治 / fallback） | 单步 prompt（CLI 驱动） |
|---|---|---|
| 长度 | ~100 行讲义 | ≤ 40 行 |
...
```
历史 Agent 自治架构：一个 LLM 全部背下来 + 自己自觉遵守 + 自报合规
CLI 驱动架构：       拆成三段归属
...
历史协议文本（SKILL.md：395 行）：

> 「新功能和缺陷修复必须使用垂直切片 TDD。每条 REQ 标为 passed 前，其验证证据中必须包含至少一个本轮新增的行为测试……」

历史 Agent 自治路径：Worker 自己读这段话，自己决定是否守规。可能它就跑了一个 happy path 测试就标 passed。
...
| 文件 / 目录 | 兼容 fallback current | `--run` 目标路径 planned | 约束 |
|---|---|---|---|
...
- **Provider-proxy smoke**：如果目标是测稳 auto-iterate 本身，而不是证明 Claude/Gemini/Cursor 官方 native 账号链路，可使用 `scripts/auto-iterate-openai-worker.js` 作为 OpenAI-compatible Worker，并通过 `AUTO_ITERATE_CLAUDE_CMD` / `AUTO_ITERATE_GEMINI_CMD` / `AUTO_ITERATE_CURSOR_CMD` 指向该脚本。该路径能验证 env-template spawn、外部模型 HTTP 调用、`result.json` 写回、CLI validation、state merge 与 NDJSON 事件，但只能标记为 `provider-proxy smoke passed`，不得替代 `native smoke passed`。本地 mock provider 覆盖命令为 `npm run smoke:provider-proxy`；真实 OpenAI-compatible provider 可设置 `OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_MODEL` 后运行 `npm run smoke:provider-proxy:real`，也可用 `AUTO_ITERATE_PROVIDER_PROXY_AGENTS=claude,cursor` 收窄矩阵。
...
3. options.run 非 true:
   不进入 pipeline，走兼容 fallback 路径（不变）
```
...
6. --no-run：即使传了 --run 也强制走兼容 fallback 大 prompt 路径，用于调试和回归对比。
```

`--run` 模式应默认非交互；Router LLM 自动路由时必须显式传 `--session <generated-session>`，但不追加 `--yes`。只有兼容 fallback 启动命令才追加 `--yes --no-run`，避免把路径 A/B 的执行语义混在一起。
...
| `budgets` | 初始预算由 CLI 生成，后续兼容 fallback 可由 Agent 更新 | `--run` 下 CLI 唯一递减 | Worker result 不允许写 budgets。 |
...
- Router LLM 只有在 `workers_available=[]`、用户显式要求 `--no-run` 或环境不能 spawn Worker 时才允许回退兼容 fallback 路径；其他错误应汇报并停止或请求用户修复环境。
...
| `--no-run --run ...` | 强制兼容 fallback 路径，用于回归对比；help 文本必须说明优先级。 |
...
| `parsed` | CLI 能识别但可能报“未实现” | `parseArgs` 有字段；非法组合有明确错误；不影响兼容 fallback 路径 |
...
| flag | 当前阶段 | 说明 |
|---|---|---|
| `--run` | `routable` | 可由 Router 在 Worker 可用时调用；不带 `--run` 的流程只作为兼容 fallback |
...
- 不传 `--run` → 仍走兼容 fallback 路径：生成 state.json + 大 prompt，打印到 stdout。
- `--validate-state` / `--finalize` / `--dispatch` / `--list` / `--switch` / `--resume`（不带 `--run`）全部保持原行为。

### 10.2 --check 发现无 Worker → 退回兼容 fallback
...
2. 退回兼容 fallback 路径：`fastcar-cli auto-iterate --from ... --yes --no-run`（生成大 prompt），当前 Agent 才接管 prompt。

这是唯一允许自动退化为 fallback 执行循环的场景；用户显式要求 `--no-run` 时也可直接进入该路径。

### 10.3 --no-run 强制兼容 fallback 路径

调试或对比时，`--no-run` 显式压制 `--run`，强制走兼容 fallback 路径。

### 10.4 迁移与回滚规则

CLI 驱动迁移必须按“CLI 驱动路径优先、兼容 fallback 可回滚”的方式推进。任何 PR 只要导致 fallback 路径不可用，就必须回滚或拆分。
...
| 回滚对象 | 必须仍然可用 | 不允许残留 |
|---|---|---|
| 回滚 PR-1 | `fastcar-cli auto-iterate --quick --goal ... --yes --no-run` 仍生成启动文件 | `parseArgs` 误吃旧 flag、help 宣传已不存在 flag |
| 回滚 PR-2 | PR-1 的 `--run --once` 如保留则仍能单轮运行；否则完全回到兼容 fallback 路径 | 半写入的 `state.json` 新字段导致 `--validate-state` 失败 |
...
| `src/pipeline/phaseGate.js` | 203 | 阶段门禁：输出当前阶段、是否可继续和多项阻断原因，并同步 `phaseGate.gates[]`，覆盖 contract、baseline、验证加固、cleanup、style consolidation、context reset、skill capture、delivery evidence 与 post-agent gate |
...
- [x] Claude/Gemini/Cursor provider-proxy mock smoke 通过：`npm run smoke:provider-proxy` 使用 `scripts/auto-iterate-openai-worker.js` 和本地 OpenAI-compatible mock server 跑通三类 agent 的 HTTP Worker 链路；`npm run smoke:provider-proxy:real` 可在提供 `OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_MODEL` 后用真实 provider 跑同一路径，证明无需官方 Claude/Gemini/Cursor 账号也能测稳 auto-iterate env-template/provider-proxy 路径；该结论不替代 native smoke。
...
- [ ] `npm test`。
- [ ] `node bin/cli.js auto-iterate --quick --goal "兼容性冒烟" --session compat-smoke --yes --no-run` 在临时目录能生成 session。
...
- [ ] `--run` 修改或删除兼容 fallback 大 prompt 路径。
...
| current/planned 边界 | 有条件通过 | §0.2 和 §0.7 已明确 8 类设计规格 gap；核心闭环可跑但细节偏离待补齐。后续增强不得破坏兼容 fallback。 |
...
3. 保持兼容 fallback 路径完全可用（✅ 当前代码中 `--no-run` / 不带 `--run` 仍走兼容 fallback 路径），但 Router LLM 默认路由必须走 `--check` 后接 `--run --json-progress`。

### 13.1 代码层

- [x] `parseArgs` 不破坏现有 flag 行为（兼容 fallback 路径保持可用，15 个 pipeline flag 已解析）。
...
- [x] **用例 4 机器规则（无 Worker 环境）**：环境里没装任何 Worker CLI。Router LLM 从 `--check` 识别出，明示告诉用户「本机未安装 Worker CLI，本次由我在当前会话里代跑」，然后才退回兼容 fallback 路径，启动命令必须显式包含 `--yes --no-run`。
...
- 不重写 `buildPromptContent`（兼容 fallback 大 prompt 模式保留）
...
本附录描述 `--run` 稳定后的目标路由。当前仓库已实现 `--run`、`--check`、`--json-progress` 和基础多轮 pipeline，Router LLM 可以在 `--check` 发现可用 Worker CLI 后使用本表命令；若 Worker 不可用、用户显式 `--no-run`，或目标能力属于 §0.7 未实现项，则回退到兼容 fallback 命令，例如 `fastcar-cli auto-iterate --quick --goal ... --yes --no-run`、`--strict --from ... --yes --no-run`、`--verify --from ... --yes --no-run`。
...
| 代码 | 位置 |
|---|---|
| 兼容 fallback 大 prompt 构造 | `src/auto-iterate.js:3885-` `buildPromptContent` |
