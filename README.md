...
`auto-iterate` 支持两种执行模式：默认推荐 CLI 驱动自动模式，Router 先检查 Worker，再由 CLI 持有主循环、状态、验证和停止条件；手动 / fallback 模式只在 Worker 不可用、用户显式要求 `--no-run` 或需要生成大 prompt 时使用。
...
fastcar-cli auto-iterate --run --once --verify --from docs/prd.md --session prd-check --json-progress
fastcar-cli auto-iterate --run --once --plan-only --goal "规划订单模块重构" --session order-plan --json-progress
...
手动 / fallback 模式会生成独立 session 状态文件和启动提示，随后由当前 Agent 按协议执行：
...
fastcar-cli auto-iterate --verify --from docs/prd.md --session prd-check --yes --no-run
fastcar-cli auto-iterate --plan-only --goal "规划订单模块重构" --session order-plan --yes --no-run
...
```bash
fastcar-cli auto-iterate --check --json-progress
fastcar-cli auto-iterate --run --autopilot --strict --from docs/ai-checklist.md --session ai-checklist-impl --json-progress
```

自动模式下不要要求用户复制 `.agent-state/auto-iterate/<session>/start-prompt.md`；Router 只读取 NDJSON 进度并在 `need_decision` 时询问用户。只有手动 / fallback 模式才把 `start-prompt.md` 作为后续执行依据。
...
2. 运行 fastcar-cli auto-iterate --check --json-progress。
...
- 自然语言路由时也要让 Agent 生成独立 session。例如“帮我快速启动自动迭代修复登录失败，session 叫 login-bugfix，最多跑 5 轮”应先路由为 `fastcar-cli auto-iterate --check --json-progress`，Worker 可用后再运行 `fastcar-cli auto-iterate --run --autopilot --quick --goal "修复登录失败" --session login-bugfix --autopilot-max-iterations 5 --json-progress`。
...
- 长任务要持续维护 `.agent-state/auto-iterate/<session>/state.json`，并刷新生成视图 `.agent-state/auto-iterate/<session>/state.md`。恢复任务时自动模式使用 `fastcar-cli auto-iterate --resume <session> --run --autopilot --json-progress`；只有手动 / fallback 模式才运行 `fastcar-cli auto-iterate --resume <session>` 后读取对应 `start-prompt.md`。
...
模板结构约定：模板包中应包含 `template/` 目录，并在 `package.json` 中声明模板包名称、版本和生产依赖。
