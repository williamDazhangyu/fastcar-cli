...
`auto-iterate` 当前有效架构是主 Agent 直接管理 coder subagent：主 Agent 选取 focus、派发 coder、读取 result、运行验证、审计 diff 并合并 state。CLI 不再持有外部 Worker 主循环；`--run`、`--check`、`--dispatch` 属于 legacy/deprecated 入口，默认会被拒绝。

CLI 主要用于生成/维护 session、只读校验 state、finalize 交付文档，以及生成 protocol-only / LLM-only session 骨架：
...
```bash
fastcar-cli auto-iterate --strict --from docs/ai-checklist.md --session ai-checklist-impl --yes
```

默认原生 subagent 模式下，主 Agent 直接读取 skill/state 并执行裁判职责。用户明确要求 protocol-only / LLM-only、手动模式或不启动 subagent 时，在启动命令追加 `--no-run`，由当前 LLM 自己遵循自动迭代技巧执行。
...
- [docs/auto-iterate-current-architecture.md](./docs/auto-iterate-current-architecture.md)：当前有效架构，说明主 Agent / coder subagent 边界、validation.log 门禁和旧 CLI Worker 路径边界。
...
- 自然语言路由时也要让 Agent 生成独立 session。例如“帮我快速启动自动迭代修复登录失败，session 叫 login-bugfix，最多跑 5 轮”应由主 Agent 原生 subagent 工作流执行；如需 session 骨架，运行 `fastcar-cli auto-iterate --quick --goal "修复登录失败" --session login-bugfix --autopilot-max-iterations 5 --yes`。如果用户明确说手动模式、protocol-only 或不要 subagent，才追加 `--no-run`。
...
- 长任务要持续维护 `.agent-state/auto-iterate/<session>/state.json`，并刷新生成视图 `.agent-state/auto-iterate/<session>/state.md`。恢复任务时使用 `fastcar-cli auto-iterate --resume <session>` 激活状态后，由主 Agent 继续原生 subagent 裁判循环。
...
# 2. 可选生成 session 骨架；默认执行由主 Agent 原生 subagent 工作流完成
fastcar-cli auto-iterate --quick --goal "修复登录失败" --session login-bugfix --autopilot-max-iterations 5 --yes

# 3. 如果用户要求 protocol-only / LLM-only，则追加 --no-run 并读取启动提示
