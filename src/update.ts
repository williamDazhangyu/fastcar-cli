 ...
 */
export async function updateTemplateTarget(packageName: string, targetDir: string): Promise<boolean> {
 ...
 */
export async function updateCosTemplate(args: string[] = []): Promise<void> {
  try {
    ...
    if (!templateConfig) {
      ...
      Object.keys(templateRegistry).forEach((key) => {
        console.error(`  • ${key} - ${templateRegistry[key].description}`);
      });
      console.error("=".repeat(50) + "\n");
      process.exit(1);
    }
