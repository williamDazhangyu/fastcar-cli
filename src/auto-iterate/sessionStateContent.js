...
exports.buildStateContent = buildStateContent;
const modeRules_1 = require("./modeRules");
const sessionConfig_1 = require("./sessionConfig");
const stateValidationHelpers_1 = require("./stateValidationHelpers");
const language_1 = require("../pipeline/language");
function buildStateContent(rawAnswers) {
    const answers = (0, sessionConfig_1.withModeDefaults)(rawAnswers);
    const lang = (0, language_1.languageCode)(answers.language);
...
成功标准：
${(0, sessionConfig_1.formatList)(answers.successCriteria)}

非目标：
${(0, sessionConfig_1.formatList)(answers.nonGoals)}

允许修改范围：
${answers.allowedScope || "未指定"}

兼容性约束：
${(0, sessionConfig_1.formatList)(answers.compatibility)}
...
gates：
${stateValidationHelpers_1.ENGINE_PHASES.map((phase) => `  - phase：${phase}
    status：${phase === "requirement" ? "pending" : "blocked"}
    entry：${(0, stateValidationHelpers_1.defaultPhaseEntryCriteria)(phase).join("；")}
    exit：${(0, stateValidationHelpers_1.defaultPhaseExitCriteria)(phase).join("；")}
    blocking：${(0, stateValidationHelpers_1.defaultPhaseBlockingRules)(phase).join("；")}`).join("\n")}
...
non_goals：${(0, sessionConfig_1.normalizeLines)(answers.nonGoals).join("；") || "未指定"}
success_criteria：${(0, sessionConfig_1.normalizeLines)(answers.successCriteria).join("；") || "未指定"}
validation_plan：${(0, sessionConfig_1.normalizeLines)(answers.validationCommands).join("；") || "未指定"}
...
## Baseline / 修改前基线
status：pending
command：${(0, sessionConfig_1.normalizeLines)(answers.validationCommands)[0] || "not_run"}
result：未运行
...
${(0, sessionConfig_1.normalizeLines)(answers.successCriteria)
...
${(0, sessionConfig_1.formatList)(answers.compatibility)}
用户提供的限制：
${(0, sessionConfig_1.formatList)(answers.constraints)}
并发决策：
...
最终交付可验证性：unknown
可运行的验证命令：
${(0, sessionConfig_1.formatList)(answers.validationCommands)}
...
status：${(0, modeRules_1.isImplementationMode)(answers.mode) ? "pending" : "not_applicable"}
...
skipped_reasons：${(0, modeRules_1.isImplementationMode)(answers.mode) ? "无" : "当前模式不是实现需求模式"}
...
目标：${answers.goal || "未指定"}
成功标准：${(0, sessionConfig_1.normalizeLines)(answers.successCriteria).join("；") || "未指定"}
