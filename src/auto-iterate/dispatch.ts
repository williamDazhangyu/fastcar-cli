...
interface DispatchState {
  ...
  subAgentHistory: StateObject[];
  dispatchedCount: number;
  completedCount: number;
  failedCount: number;
  lastDispatchRound: number;
...
const DISPATCH_AGENT_ALIAS_MAP = Object.entries(DISPATCH_AGENT_CONFIGS).reduce<Record<string, string>>(
  (aliases, [key, config]) => {
...
export function normalizeDispatchAgent(value: unknown): string | null {
  const normalized = String(value || "codex").trim().toLowerCase();
  return DISPATCH_AGENT_ALIAS_MAP[normalized] || null;
}

export function makeAgentId(agent: string | null | undefined, session: string | null | undefined): string {
...
export function selectVerifyCommand(stateJson: StateObject | null | undefined, fallback: string | null | undefined): string {
  if (fallback) {
    return fallback;
  }
...
export async function createDispatchWorktree(sessionPaths: SessionPaths, agentId: string): Promise<string> {
...
export function formatActiveSubAgentsBlock(agents: ActiveSubAgent[] | null | undefined): string {
...
export function normalizeSubAgentHistory(history: unknown): StateObject[] {
  return (Array.isArray(history) ? history : [])
    .filter((item): item is StateObject => Boolean(item && typeof item === "object"))
    .slice(-MAX_SUB_AGENT_HISTORY_ITEMS);
}

export function formatSubAgentHistoryBlock(history: unknown): string {
...
function getExistingSubAgentDispatch(stateJson: StateObject | null | undefined): StateObject {
  return stateJson && stateJson.subAgentDispatch && typeof stateJson.subAgentDispatch === "object"
    ? stateJson.subAgentDispatch
    : {};
}

export function buildDispatchCounters(
  existingDispatch: StateObject | null | undefined,
  activeIncrement = 1,
): { dispatchedCount: number; completedCount: number; failedCount: number; lastDispatchRound: number } {
  return {
...
function replaceSection(content: string, heading: string, nextHeadingPattern: string, replacementBody: string): string {
...
export function updateStateMarkdownForDispatch(content: string, dispatch: DispatchState): string {
...
export function updateDecisionsMarkdownForDispatch(content: string, dispatch: DispatchState): string {
  return content
    .replace(
...
export function updateStateJsonForDispatch(stateJson: StateObject, dispatch: DispatchState): StateObject {
...
export function hasUnmergedActiveSubAgents(stateJson: StateObject | null | undefined): boolean {
  const active = stateJson &&
    stateJson.subAgentDispatch &&
    Array.isArray(stateJson.subAgentDispatch.activeSubAgents)
    ? stateJson.subAgentDispatch.activeSubAgents
    : [];
  return active.some((agent: StateObject) => agent && agent.mergeStatus !== "merged" && agent.mergeStatus !== "skipped");
}

export async function initDispatch(options: DispatchOptions): Promise<void> {
  ...
  const finishedAgent: ActiveSubAgent = {
    ...activeAgent,
  ...
  const afterRunStateJson = await readJsonFile(sessionPaths.sessionStateJsonPath) as StateObject | null;
