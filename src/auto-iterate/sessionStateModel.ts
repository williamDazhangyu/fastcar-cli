...
export function buildStateModel(rawAnswers: StateObject): StateObject {
  ...
  return {
    ...
    requirements: [
      {
        ...
        userVisibleBehavior: "用户目标被拆成可独立推进、验证和交付的薄需求条目",
        expectedBehavior: "每条 REQ 都优先描述用户可见行为，并记录必要的预期、实际、复现、验收影响和依赖信息",
        actualBehavior: "尚未提取完整 Requirement Coverage Matrix",
        reproSteps: ["读取用户目标", "读取成功标准或来源文档", "只读探索现有行为和领域语言", "拆分 REQ-001...REQ-N"],
        acceptanceImpact: "未完成 RCM 前不得进入实现或声称交付完成",
        type: "验证",
        status: "pending",
        dependsOn: [],
        blockedBy: [],
    ...
    skillCapture: {
      ...
      pendingCandidates: [],
      skippedReasons: [],
      selectionCriteria: "只沉淀可迁移、可行动、可验证的技能点；按 Trigger / Signal、Do、Verify、Avoid、Boundary、Source Evidence 结构压缩；不要记录密钥、客户数据、一次性日志、完整报错堆栈或大段源码",
