import { addError, type ValidationIssue } from "./stateValidationPrimitives";
...
const ENGINE_PHASE_SET = new Set<string>(ENGINE_PHASES);

export type EnginePhase = "requirement" | "contract" | "baseline" | "coding" | "validation" | "cleanup" | "delivery";

export interface RequirementStateCounts {
  passed: number;
  pending: number;
  implemented: number;
  notVerified: number;
...
type PhaseCriteriaMap = Record<EnginePhase, string[]>;

export function isEnginePhase(phase: string): phase is EnginePhase {
...
export function countJsonRequirementStates(requirements: unknown[]): RequirementStateCounts {
  const counts = {
    passed: 0,
    pending: 0,
    implemented: 0,
...
export function defaultPhaseEntryCriteria(phase: string): string[] {
  const criteria: PhaseCriteriaMap = {
...
export function defaultPhaseExitCriteria(phase: string): string[] {
  const criteria: PhaseCriteriaMap = {
...
export function defaultPhaseBlockingRules(phase: string): string[] {
  const rules: PhaseCriteriaMap = {
...
export function hasValidatedBaseline(baseline: StateObject | null | undefined): boolean {
  if (!baseline) {
    return false;
  }
...
export function hasOpenRequirementCounts(counts: RequirementStateCounts): boolean {
