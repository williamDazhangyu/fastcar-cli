...
const fs = require("fs");
const { getSessionPaths, toRelative } = require("./sessionPaths");
const {
  addError,
  addWarning,
...
const {
  ...
  parseStateNumber,
  stateHeadingExists,
} = require("./stateMarkdownParsers");
const { compareCurrentPointerToExpected } = require("./sessionStateValidation");

/**
 * @typedef {import("./stateValidationPrimitives").ValidationIssue} ValidationIssue
 * @typedef {import("./sessionStateValidation").StateFileValidationTarget} StateFileValidationTarget
 * @typedef {{ passed: number, pending: number, implemented: number, notVerified: number, blocked: number }} RequirementStateCounts
 * @typedef {{ issues: ValidationIssue[] }} SessionBaselineValidationResult
 */
 ...
 * @param {StateFileValidationTarget} stateInfo
 * @returns {Promise<SessionBaselineValidationResult>}
 */
