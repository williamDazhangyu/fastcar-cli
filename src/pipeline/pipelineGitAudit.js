...
const { normalizeRelativePath } = require("./resultSchema");
 ...
 * @returns {string}
 */
function getDirectorySignature(root, absolute, status) {
  ...
  /** @type {Array<{ filePath: string; index: number; baseEntry: string }>} */
  const fileEntries = [];
  /** @param {string} entry */
 ...
 * @returns {string}
 */
function getFileSignature(cwd, file, status) {
 ...
 * @returns {void}
 */
function addInternalAgentStateChildren(cwd, directory, status, files) {
 ...
 */
function getGitStatusSnapshot(cwd, signaturePaths = null) {
 ...
 * @returns {string[]}
 */
function diffStatusSnapshots(before, after) {
 ...
 */
function mergeActualFilesChanged(result, actualFiles) {
 ...
 * @returns {boolean}
 */
function isAllowedInternalActualWrite(file, allowedInternalWrites) {
 ...
 * @returns {string[]}
 */
function normalizeActualFilesChanged(files, allowedInternalWrites) {
  /** @type {string[]} */
