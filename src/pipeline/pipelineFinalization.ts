...
import { asArray, asRecord, nonEmptyString } from "./valueUtils";

interface RequirementClosure {
  open: string[];
  blocked: string[];
  allPassed: boolean;
}

interface FinalizeDeliveryOptions {
  session?: string;
  mode?: string;
  reason?: string;
}

interface FinalizeDeliveryResult {
 ...
 */
export function requirementClosure(state: PipelineStateLike): RequirementClosure {
 ...
 * @returns {string[]}
 */
function collectChangedFiles(state: PipelineStateLike): string[] {
 ...
 * @returns {string}
 */
function validationSummary(state: PipelineStateLike): string {
 ...
 * @returns {boolean}
 */
export function canFinalizeDelivery(state: PipelineStateLike): boolean {
  const deliveryEvidence = asRecord(state.deliveryEvidence);
 ...
 */
export function finalizeDeliveryState(
  state: PipelineStateLike,
  options: FinalizeDeliveryOptions = {},
  ...
  const rawBudgets = asRecord(state.budgets);
  const budgets = rawBudgets as PipelineBudgets & Record<string, unknown>;
  const currentWatchdog = asRecord(state.watchdog);

  const next: PipelineStateLike = {
    ...state,
    updatedAt: now,
    budgets: {
      ...budgets,
