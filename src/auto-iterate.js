...
const DEFAULT_DELIVERY_FORMAT =
  "最终输出实现总结、关键修改、完整任务清单完成状态、需求覆盖矩阵（Requirement Coverage Matrix）、完成定义（Definition of Done）、验证证据、未验证项、剩余需求、风险、验收建议，以及 .agent-state/auto-iterate-coding.md 的最终状态摘要。";
...
function getStatePaths() {
  ...
  return {
    ...
    legacyStatePath: path.join(stateDir, STATE_FILE),
    legacyPromptPath: path.join(stateDir, PROMPT_FILE),
  };
}
...
async function writeCurrentFile(sessionPaths, answers) {
  const current = {
    ...
    legacyStateFile: toRelative(sessionPaths.legacyStatePath),
    legacyPromptFile: toRelative(sessionPaths.legacyPromptPath),
  };
...
async function syncLatestFiles(sessionPaths) {
  await fs.promises.copyFile(
    sessionPaths.sessionStatePath,
    sessionPaths.legacyStatePath,
  );
  await fs.promises.copyFile(
    sessionPaths.sessionPromptPath,
    sessionPaths.legacyPromptPath,
  );
}
...
async function activateSession(sessionName, action = "switch") {
  ...
  await syncLatestFiles(sessionPaths);
  ...
  console.log(`   Latest 状态镜像: ${toRelative(sessionPaths.legacyStatePath)}`);
  console.log("\n下一步:");
...
legacy 镜像：${answers.legacyStateFile || ".agent-state/auto-iterate-coding.md"} / ${answers.legacyPromptFile || ".agent-state/auto-iterate-start-prompt.md"}
恢复优先级：session state 优先；legacy 镜像仅用于兼容旧流程
...
类型：验证
状态：pending
相关文件：${answers.sessionStateFile || ".agent-state/auto-iterate-coding.md"}
验证证据：无
阻塞原因：无
...
如果 session state 和 legacy latest 镜像同时存在，请以 session state 为准。
...
请不要依赖历史对话作为唯一上下文。
如果存在 ${answers.sessionStateFile || ".agent-state/auto-iterate/default/state.md"}，请先读取它作为本 session 的恢复状态；如果只存在 legacy 镜像 .agent-state/auto-iterate-coding.md，再读取 legacy 镜像作为兼容恢复状态。
...
async function initAutoIterate(args = []) {
  ...
  console.log("可选择严格启动、快速启动、Verify-only、Plan-only 或 Optimization-only。");
  console.log("也可以使用: fastcar-cli auto-iterate --from <清单文档路径>\n");

  const mode = await resolveMode(options);
  if (!mode || !MODE_CONFIGS[mode]) {
    console.log("❌ 无效启动模式，请使用 strict / quick / verify / plan / optimize");
    return;
  }
